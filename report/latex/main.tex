...
\begin{table}[h]
  ...
  \begin{tabular}{p{3.4cm}p{2.0cm}p{8.3cm}}
    ...
    Do Tuan Minh & 20261057M &
      Team lead, project architect, and final integrator. Conceived the overall
      research direction and framed the central question of the study (structure vs.\
      no-structure, and which architectural choices matter); designed the unified
      experimental protocol---the five-seed evaluation, the best-validation
      model-selection rule, and the ``mean~$\pm$~std'' reporting standard---that makes
      every result in the paper comparable and reproducible. Established the modular
      code architecture (the model registry, shared training loop, and one-command
      experiment scripts) so that all four teammates could plug in their models against
      a common backbone, and defined the experiment-table and figure formats adopted
      throughout. Authored the Abstract, Introduction (\S1), Problem Formulation and
      Dataset (\S2), and the Background message-passing framework (\S3) that unifies all
      models; coordinated and reviewed every section for technical accuracy and
      narrative coherence, and carried out the final scientific editing and quality
      control of the complete manuscript. Owned the reproducibility deliverables
      end-to-end: assembled and maintained the GitHub repository, the runnable
      Colab notebook, and the camera-ready NeurIPS-style LaTeX build. \\
    \addlinespace
